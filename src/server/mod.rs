...
async fn apply_named_group_metadata_event_inner(
    ...
    match event {
        ...
        NamedGroupMetadataEvent::MemberJoined {
            ...
            //       a fresh invite.
            //    b) Returning-member re-key — the member reinstalled / lost local
            //       TreeKEM state and re-issues MemberJoined with a NEW
            //       KeyPackage + a fresh single-use invite. They stay
            //       roster-active but keyless (x0xd 424 "TreeKEM group not
            //       loaded") until we re-key. Perform an MLS remove+add at the
            //       current epoch so they receive a fresh Welcome.
            if info.has_active_member(&member_agent_id) {
                ...
                if guard.epoch() != expected1 {
                    tracing::warn!(
                        group_id = %LogHexId::group(&resolved_group_key),
                        member = %LogHexId::agent(&member_agent_id),
                        "MemberJoined re-key: remove epoch mismatch"
                    );
                    return false;
                }
                ...
                if guard.epoch() != expected2 {
                    tracing::warn!(
                        group_id = %LogHexId::group(&resolved_group_key),
                        member = %LogHexId::agent(&member_agent_id),
                        "MemberJoined re-key: add epoch mismatch"
                    );
                    return false;
                }

                // --- Persist ONCE after both commits, then expose roster ---
...
/// arm): only a `MemberAdded`, only for this node (member==self), only from
/// the recorded expected inviter (ADR-0016: adds are inviter-authored by any
/// active admin, so `validate_join_result_inviter` gates on the admin whose
/// invite this join consumed); then `apply_named_group_metadata_event` (with
...
/// forged result fails the inviter check or the commit verify. Token-gated by
...
async fn apply_join_result_endpoint(
    ...
    // ADR-0016 port: group adds are inviter-authored by any active admin, so
    // the authority gate is the recorded expected inviter (the admin whose
    ...
    // runs — not the retired creator gate.
