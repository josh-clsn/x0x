...
/// Default credit of concurrent inbound unidirectional QUIC streams
/// advertised per connection. ant-quic materializes a streams-table slot for
/// every advertised stream at `Connection` construction, so this bound sets a
/// per-connection memory floor; 4_096 keeps over 4x headroom above the
/// concurrency the X0X-0063 soak measured at roughly 1/12 the memory of the
/// former 50k credit. See the ant-quic builder site in `NetworkNode::new`.
pub const DEFAULT_MAX_CONCURRENT_UNI_STREAMS: u32 = 4_096;
...
impl NetworkNode {
    ...
    pub async fn new(
        ...
        let mut builder = NodeConfig::builder()
            ...
            // the proper fix. It is also sized for chat nodes with RAM to
            // spare: a public-facing relay candidate should size the datagram
            // channel far lower, because at 50k the queued inbound datagrams
            // this ceiling permits dominate RSS (the 2026-07 relay-droplet
            // exhaustion; a dhat heap census attributed 58% of live heap to
            // them). Both bounds therefore read from `NetworkConfig`, and
            // both default to the values below, so a deployment that sets
            // neither TOML key behaves exactly as before.
...
mod message_tests {
    ...
    fn transport_bounds_parse_from_toml() {
        let config: NetworkConfig =
            toml::from_str("max_concurrent_uni_streams = 256\ndata_channel_capacity = 2048\n")
