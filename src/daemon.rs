//! Daemon server bring-up for the x0x gossip network.
//!
//! This module owns the entire `x0xd` server: configuration, shared state,
//! the REST + WebSocket route handlers, the background tasks, and the axum
//! serve loop. The `x0xd` binary (`src/bin/x0xd.rs`) is a thin wrapper that
//! calls [`run`]; embedders (e.g. a mobile FFI crate that cannot reliably
//! supervise a child daemon process) call [`serve`] to bring the same server
//! up in-process and drive it through the returned [`ServerHandle`].
//!
//! ## CLI usage (via the `x0xd` binary)
//!
//! ```bash
//! x0xd                                  # default config
...
// This module was lifted verbatim out of `src/bin/x0xd.rs` so the server
// bring-up could become a reusable library entrypoint (issue #110). All of
// the moved code refers to crate items by the crate's own name (`x0x::...`);
// `extern crate self as x0x;` keeps those paths resolving now that the code
// lives inside the crate, so the move stayed mechanical and string literals
// such as the `target: "x0x::diag::..."` tracing targets are untouched.
extern crate self as x0x;
...
// The global allocator selection (jemalloc / dhat) and the `malloc_conf`
// export are binary-level concerns and live in `src/bin/x0xd.rs`, not here.
...
///
/// `pub` so in-process embedders can build one and pass it to [`serve`].
/// `Default::default()` yields the same defaults the binary uses when no
...
///
/// `pub` because it is a field of the `pub` [`DaemonConfig`].
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct DaemonUpdateConfig {
    ...
    pub rollout_window_minutes: u64,
    ...
    pub fallback_check_interval_minutes: u64,
...
/// Run the `x0xd` daemon as a command-line program.
///
/// This owns the CLI-specific concerns: argument parsing, building the
/// [`DaemonConfig`], logging init, the `--doctor` / `--check` /
/// `--check-updates` / `--list` early-exit modes, and the startup self-update
/// check. Once the config is built it hands off to [`serve`] for the actual
/// server bring-up, then reproduces the binary's Ctrl+C / `/shutdown`
/// graceful-shutdown behaviour via the returned [`ServerHandle`].
///
/// The `x0xd` binary (`src/bin/x0xd.rs`) is a thin wrapper around this.
/// In-process embedders (e.g. a mobile FFI crate) should call [`serve`]
/// directly instead, supplying their own [`DaemonConfig`] and exec policy.
pub async fn run() -> Result<()> {
    ...
    // Fold the remaining CLI-only override into the config so that `serve`
    // depends on the config alone for the port-mapping decision (the
    // `--no-port-mapping` flag wins over the config TOML, matching the
    // previous inline `config.port_mapping_enabled && !cli_no_port_mapping`).
    if cli_no_port_mapping {
        config.port_mapping_enabled = false;
    }

    let mut handle = serve(config, exec_policy, cli_disable_peer_cache).await?;

    // Reproduce the binary's previous shutdown behaviour exactly: wait for
    // either Ctrl+C or an HTTP `/shutdown` request, then drive the graceful
    // shutdown + cleanup via the handle.
    tokio::select! {
        _ = signal::ctrl_c() => {
            tracing::info!("Received Ctrl+C shutdown signal");
        }
        _ = handle.api_shutdown_requested() => {
...
/// A running daemon server returned by [`serve`].
///
/// Holds the bound API address (read back from the listener so callers that
/// bound an ephemeral port -- e.g. `127.0.0.1:0` on mobile -- learn the actual
/// port), a graceful-shutdown trigger, a way to observe an HTTP `/shutdown`
/// request, and the join handle for the spawned `axum::serve` task.
pub struct ServerHandle {
    local_addr: SocketAddr,
    /// Shared server state. Retained so the handle can trigger the watch-based
    /// graceful-shutdown signal (`state.shutdown_notify`), expose the running
    /// agent, and shut the agent down on teardown -- exactly as the binary's
    /// `main` did inline before this refactor.
    state: Arc<AppState>,
    /// Fires when a client hits the HTTP `/shutdown` endpoint. Held here so an
    /// embedder (or the binary) can `select!` on it alongside its own signals.
    api_shutdown_rx: mpsc::Receiver<()>,
    /// The spawned `axum::serve(...)` task.
    server_task: tokio::task::JoinHandle<std::io::Result<()>>,
    /// `<data_dir>/api.port`, written during bring-up and removed on shutdown.
    port_file: PathBuf,
}

impl ServerHandle {
    /// The address the API server is actually bound to.
    ///
    /// When the config requested an ephemeral port (`:0`) this reports the OS
    /// assigned port, which is the value embedders must read back.
    ...
    /// Trigger a graceful shutdown without consuming the handle.
    ///
    /// Sending on the watch channel makes the serve task's
    /// `with_graceful_shutdown` future resolve and is observed by every
    /// background task subscribed to it. Call [`ServerHandle::join`] afterwards
    /// to await full teardown.
    pub fn shutdown(&self) {
        let _ = self.state.shutdown_notify.send(true);
    }

    /// Resolve when a client requests shutdown via the HTTP `/shutdown`
    /// endpoint. Resolves immediately if the sender has been dropped.
    pub async fn api_shutdown_requested(&mut self) {
        let _ = self.api_shutdown_rx.recv().await;
    }

    /// Trigger graceful shutdown (idempotent), await the server task with the
    /// same 2 second budget the binary used, clean up the port file, and shut
    /// the agent down. Returns the server task's result.
    pub async fn join(mut self) -> Result<()> {
        ...
        self.state.agent.shutdown().await;
...
/// `axum::serve` loop: it derives the bind address and instance-scoped
/// identity directory from `config`, builds the [`Agent`], binds the API
/// listener (reading back the actual address), spawns every background task,
/// builds the router, writes the `api.port` file, and spawns the serve future
/// wired to the handle's graceful-shutdown trigger. It does NOT block on the
/// server -- the serve future runs on a spawned task so the caller keeps
/// control.
///
/// `exec_policy` is passed in (rather than loaded here) because the binary
/// resolves it from the `--exec-acl` CLI flag; embedders build their own.
/// `disable_peer_cache` mirrors the binary's `--disable-peer-cache` flag.
pub async fn serve(
    config: DaemonConfig,
    exec_policy: x0x::exec::ExecPolicy,
    disable_peer_cache: bool,
) -> Result<ServerHandle> {
    let cli_disable_peer_cache = disable_peer_cache;
    // Resolve the instance name from the config. `run` writes the CLI
    // `--name` value back into `config.instance_name` before calling us, so
    // this is the single source of truth for both the binary and embedders.
    ...
    let network_config = NetworkConfig {
        ...
        max_peers_per_ip: 3,
        // The `--no-port-mapping` CLI flag is already folded into
        // `config.port_mapping_enabled` by `run` before `serve` is called, so
        // operators still override the config TOML on a single invocation.
        port_mapping_enabled: config.port_mapping_enabled,
    };
    ...
    // Hand control back to the caller. The serve future runs on `server_task`;
    // the caller drives graceful shutdown through the handle. `shutdown_rx` is
    // the receiver half of the HTTP `/shutdown` mpsc channel whose sender lives
    // in `AppState`, so it fires when a client hits `/shutdown`.
    Ok(ServerHandle {
        local_addr: actual_api_addr,
        state,
        api_shutdown_rx: shutdown_rx,
        server_task,
        port_file,
    })
}
