...
// ===========================================================================
// 5b. Join returns the joiner's minted member_joined inline (engine-A)
// ===========================================================================

/// Engine-A: the `/groups/join` handler must return the joiner's own minted
/// `member_joined` event INLINE in the response, so a relay-bridged join can
/// read it directly instead of racing the gossip SSE stream (which starves
/// whenever the joiner's gossip mesh has not formed — the cold/NAT case
/// engine A targets). Asserts the join response carries
/// `member_joined { topic, event_b64 }` and that `event_b64` base64-decodes
/// to a signed `NamedGroupMetadataEvent::MemberJoined` naming the joiner —
/// byte-identical to what a gossip capture would have yielded.
#[tokio::test]
#[ignore]
async fn named_group_join_returns_inline_member_joined() {
    ...
    // Engine-A invariant: member_joined is present inline, not null.
